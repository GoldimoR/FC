...
Gyro.AddBKB = Menu.AddOption({"Hero by Rasta", "Gyrocopter"}, "BKB", "Use BKB on combo")
Gyro.AddUltimate = Menu.AddOption({"Hero by Rasta", "Gyrocopter"}, "Ultimate", "Use ultimate in combo")
...
function Gyro.OnUpdate()
	...
	if Menu.IsKeyDown(Gyro.PressComboKey) then	
		...
		if enemyTarget and homing and Ability.IsReady(homing) and Ability.IsCastable(homing, myMana) then
		...
		if enemyTarget and Menu.IsEnabled(Gyro.AddBKB) and Ability.IsReady(BKB) and Ability.IsCastable(BKB, myMana) then
		...
		if calldown and  Menu.IsEnabled(Gyro.AddUltimate) and Ability.IsCastable(calldown, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		...
		if enemyTarget and cannon and Ability.IsReady(cannon) and Ability.IsCastable(cannon, myMana) then
		...
		if barrage and Ability.IsCastable(barrage, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),380,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
