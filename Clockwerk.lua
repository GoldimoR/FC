...
Clockwerk.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Clockwerk"}, "1.Enable", "Turn on/off this script.")
Clockwerk.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Clockwerk" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Clockwerk"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Clockwerk"}, "Kill Steal", "Auto cast Edge")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Clockwerk.npcName = "npc_dota_hero_rattletrap"

function Clockwerk.OnUpdate()
	if not Menu.IsEnabled(Clockwerk.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Clockwerk.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Clockwerk.KillSteal(myHero)
	end
	
	
	if Menu.IsKeyDown(Clockwerk.PressComboKey) then	
		-- Skills
		local battery = NPC.GetAbility(myHero, "rattletrap_battery_assault")
		local cogs = NPC.GetAbility(myHero, "rattletrap_power_cogs")  
		local rocket = NPC.GetAbility(myHero, "rattletrap_rocket_flare")  
		local Hook = NPC.GetAbility(myHero, "rattletrap_hookshot")   
		...
		if Hook and Ability.IsCastable(Hook, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(Hook,enemyAbsOrigin) 
		return end
		...
		if enemyTarget and battery and Ability.IsReady(battery) and Ability.IsCastable(battery, myMana) then
			Ability.CastNoTarget(battery) 
		return end
		
		if enemyTarget and cogs and Ability.IsReady(cogs) and Ability.IsCastable(cogs, myMana) then
			Ability.CastNoTarget(cogs) 
		return end

		if rocket and Ability.IsCastable(rocket, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(rocket,enemyAbsOrigin) 
		return end
...
function Clockwerk.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_rattletrap" then return end


    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "rattletrap_rocket_flare")
...
function Clockwerk.KillSteal(myHero)
