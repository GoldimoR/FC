...
PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Bounty Hunter" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Bounty Hunter"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Bounty Hunter"}, "Kill Steal", "Auto cast Shuriken")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Hunter.npcName = "npc_dota_hero_bounty_hunter"

function Hunter.OnUpdate()
	if not Menu.IsEnabled(optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Hunter.npcName then return end
	
	if Menu.IsEnabled(optionKillSteal) then
    Hunter.KillSteal(myHero)
	end
	...
	if Menu.IsKeyDown(PressComboKey) then	
		...
		if enemyTarget and track and Ability.IsReady(track) and Ability.IsCastable(track, myMana) then
			Ability.CastTarget(track, enemyTarget) 
		return end			
		...
		if enemyTarget and shuriken and Ability.IsReady(shuriken) and Ability.IsCastable(shuriken, myMana) then
			Ability.CastTarget(shuriken, enemyTarget) 
		return end
...
function Hunter.OnDraw()
    ...
    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "bounty_hunter_shuriken_toss")
...
function Hunter.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "bounty_hunter_shuriken_toss")
    if not edge or not Ability.IsCastable(edge, NPC.GetMana(myHero)) then return end
    local damage = Ability.GetLevelSpecialValueFor(edge, "bonus_damage")
    local range = 380
