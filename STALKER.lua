...
balanar.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Balanar"}, "1.Enable", "Turn on/off this script.")
balanar.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Balanar" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Balanar"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Balanar"}, "Kill Steal", "Auto cast VOID")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
balanar.npcName = "npc_dota_hero_night_stalker"

function balanar.OnUpdate()
	if not Menu.IsEnabled(balanar.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= balanar.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    balanar.KillSteal(myHero)
	end
	
	
	if Menu.IsKeyDown(balanar.PressComboKey) then	
		-- Skills
		local void = NPC.GetAbility(myHero, "night_stalker_void")
		local fear = NPC.GetAbility(myHero, "night_stalker_crippling_fear")  
		local hunter = NPC.GetAbility(myHero, "night_stalker_hunter_in_the_night")  
		local darkness = NPC.GetAbility(myHero, "night_stalker_darkness")  
		...
		if enemyTarget and darkness and Ability.IsReady(darkness) and Ability.IsCastable(darkness, myMana) then 
			Ability.CastNoTarget(darkness) 
		return end
		
		if enemyTarget and hunter and Ability.IsReady(hunter) and Ability.IsCastable(hunter, myMana) then 
			Ability.CastNoTarget(hunter) 
		return end
		
		if enemyTarget and fear and Ability.IsReady(fear) and Ability.IsCastable(fear, myMana) then 
			Ability.CastTarget(fear, enemyTarget) 
		return end
		
		if enemyTarget and void and Ability.IsReady(void) and Ability.IsCastable(void, myMana) then 
			Ability.CastTarget(void, enemyTarget) 
		return end
...
function balanar.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_night_stalker" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "night_stalker_void")
...
function balanar.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "night_stalker_void")
