...
optionEnableScript = Menu.AddOption({"Hero by Rasta", "Alchemist"}, "1.Enable", "Turn on/off this script.")
PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Alchemist" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
...
function Alchemist.OnUpdate()
	...
	if Menu.IsKeyDown(PressComboKey) then	
		...
		if enemyTarget and rage and Ability.IsReady(rage) and Ability.IsCastable(rage, myMana) then 
		...
		if spray and Ability.IsCastable(spray, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
