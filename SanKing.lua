...
King.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Sand King"}, "1.Enable", "Turn on/off this script.")
King.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Sand King" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Sand King"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Sand King"}, "Kill Steal", "Auto cast Burrowstrike")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
King.npcName = "npc_dota_hero_sand_king"

function King.OnUpdate()
	if not Menu.IsEnabled(King.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= King.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    King.KillSteal(myHero)
	end
	
	
	if Menu.IsKeyDown(King.PressComboKey) then	
		-- Skills
		local burrowstrike = NPC.GetAbility(myHero, "sandking_burrowstrike")
		local abilradius = Ability.GetCastRange(burrowstrike)
		-- Items
		...
		if burrowstrike and Ability.IsCastable(burrowstrike, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),abilradius) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastTarget(burrowstrike,enemyTarget) 
		return end
...
function King.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_sand_king" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "sandking_burrowstrike")
...
function King.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "sandking_burrowstrike")
