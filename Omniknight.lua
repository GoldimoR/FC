...
local optionUseUlti = Menu.AddOption({"Hero by Rasta", "Omniknight"},"Use Ultimate", "Use Guardian Angel on combo")
local optionUseRepel = Menu.AddOption({"Hero by Rasta", "Omniknight"},"Use Repel", "Use Repel on combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Omniknight"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Omniknight"}, "Kill Steal", "Cast purification on allies to KS")
local optionAutoSave = Menu.AddOption({"Hero by Rasta", "Omniknight"}, "Auto Save", "Auto save allies with purification or repel")
...
function Omniknight.OnUpdate()
	...
	if Menu.IsEnabled(optionUseUlti) and Menu.IsKeyDown(Omniknight.PressComboKey) then 
	   Omniknight.UseUlti()
	end
	
	if Menu.IsEnabled(optionUseRepel) and Menu.IsKeyDown(Omniknight.PressComboKey) then 
	   Omniknight.UseRepel()
	end   
