...
Bara.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Spirit Breaker"}, "1.Enable", "Turn on/off this script.")
Bara.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Spirit Breaker" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Spirit Breaker"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Spirit Breaker"}, "Kill Steal", "Auto cast Nether Strike")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Bara.npcName = "npc_dota_hero_spirit_breaker"

function Bara.OnUpdate()
	if not Menu.IsEnabled(Bara.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Bara.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Bara.KillSteal(myHero)
	end
	
	
	if Menu.IsKeyDown(Bara.PressComboKey) then	
		-- Skills
		local charge = NPC.GetAbility(myHero, "spirit_breaker_charge_of_darkness")
		local haste = NPC.GetAbility(myHero, "spirit_breaker_empowering_haste")  
		...
		if enemyTarget and charge and Ability.IsReady(charge) and Ability.IsCastable(charge, myMana) then
			Ability.CastTarget(charge, enemyTarget) 
		return end
		
		if enemyTarget and haste and Ability.IsReady(haste) and Ability.IsCastable(haste, myMana) then
			Ability.CastNoTarget(haste) 
		return end
...
function Bara.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_spirit_breaker" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "spirit_breaker_nether_strike")
...
function Bara.KillSteal(myHero)
    local myHero = Heroes.GetLocal()
    local edge = NPC.GetAbility(myHero, "spirit_breaker_nether_strike")
