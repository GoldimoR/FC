...
Earthshaker.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Earthshaker"}, "1.Enable", "Turn on/off this script.")
Earthshaker.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta", "Earthshaker" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Earthshaker"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Earthshaker"}, "Kill Steal", "Auto cast Breathe fire")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Earthshaker.npcName = "npc_dota_hero_earthshaker"



function Earthshaker.OnUpdate()
	if not Menu.IsEnabled(Earthshaker.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Earthshaker.npcName then return end
	
	
	
	
	if Menu.IsKeyDown(Earthshaker.PressComboKey) then	
		-- Skills
		local fissure = NPC.GetAbility(myHero, "earthshaker_fissure")
		local totem = NPC.GetAbility(myHero, "earthshaker_enchant_totem")  
		local ulti = NPC.GetAbility(myHero, "earthshaker_echo_slam")
		...
		if enemyTarget and ulti and Ability.IsReady(ulti) and Ability.IsCastable(ulti, myMana) then
		...
		if enemyTarget and totem and Ability.IsReady(totem) and Ability.IsCastable(totem, myMana) then
			Ability.CastNoTarget(totem) 
		return end
		
		if enemyTarget and fissure and Ability.IsReady(fissure) and Ability.IsCastable(fissure, myMana) then
			local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
			Ability.CastPosition(fissure, enemyAbsOrigin)
		return end
...
function Earthshaker.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_earthshaker" then return end


    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "earthshaker_fissure")
...
function Earthshaker.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "earthshaker_fissure")
