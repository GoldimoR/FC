...
optionEnableScript = Menu.AddOption({"Hero by Rasta", "Bloodseeker"}, "1.Enable", "Turn on/off this script.")
PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Bloodseeker" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Bloodseeker"}, "Awareness", "Show how many hits left to kill enemy")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Blood.npcName = "npc_dota_hero_bloodseeker"

function Blood.OnUpdate()
	if not Menu.IsEnabled(optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Blood.npcName then return end
	

	if Menu.IsKeyDown(PressComboKey) then	
		-- Skills
		local Bloodrage = NPC.GetAbility(myHero, "bloodseeker_bloodrage")
		local BloodRite = NPC.GetAbility(myHero, "bloodseeker_blood_bath")
		local Rupture = NPC.GetAbility(myHero, "bloodseeker_rupture")		
		-- Items
		...
		if enemyTarget and Rupture and Ability.IsReady(Rupture) and Ability.IsCastable(Rupture, myMana) then
			Ability.CastTarget(Rupture, enemyTarget) 
		return end
		...
		if enemyTarget and Bloodrage and Ability.IsReady(Bloodrage) and Ability.IsCastable(Bloodrage, myMana) then
			Ability.CastTarget(Bloodrage, myHero) 
		return end
		...
		if BloodRite and Ability.IsCastable(BloodRite, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
		Ability.CastPosition(BloodRite,enemyAbsOrigin) 
		return end
...
function Blood.OnDraw()
	if not Menu.IsEnabled(optionAwareness) then return end

    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_bloodseeker" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "bloodseeker_bloodrage")
