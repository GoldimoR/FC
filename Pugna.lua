...
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Pugna"}, "Kill Steal", "Auto cast Life Drain")
...
function Pugna.OnUpdate()
	...
	if Menu.IsKeyDown(Pugna.PressComboKey) then	
		...
		if enemyTarget and decrepify and Ability.IsReady(decrepify) and Ability.IsCastable(decrepify, myMana) then 
			Ability.CastTarget(decrepify, enemyTarget) 
		return end
		
		if blast and Ability.IsCastable(blast, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1200,0) then 
		local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
