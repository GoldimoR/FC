...
Sniper.optionEnableScript = Menu.AddOption({"Hero by Rasta", "Sniper"}, "1.Enable", "Turn on/off this script.")
Sniper.PressComboKey = Menu.AddKeyOption({ "Hero by Rasta","Sniper" }, "2. Execute combo key", Enum.ButtonCode.KEY_I)
local optionShrapnel = Menu.AddOption({"Hero by Rasta", "Sniper"}, "Shrapnel", "Use Shrapnel in combo")
local optionAwareness = Menu.AddOption({"Hero by Rasta", "Sniper"}, "Awareness", "Show how many hits left to kill enemy")
local optionKillSteal = Menu.AddOption({"Hero by Rasta", "Sniper"}, "Kill Steal", "Auto cast Assassinate")
local font = Renderer.LoadFont("Tahoma", 30, Enum.FontWeight.EXTRABOLD)
Sniper.npcName = "npc_dota_hero_sniper"

function Sniper.OnUpdate()
	if not Menu.IsEnabled(Sniper.optionEnableScript) then return true end
    local myHero = Heroes.GetLocal()
	if not myHero then return end
	if NPC.GetUnitName(myHero) ~= Sniper.npcName then return end
	
    if Menu.IsEnabled(optionKillSteal) then
    Sniper.KillSteal(myHero)
	end
	
    if Menu.IsEnabled(optionShrapnel) and Menu.IsKeyDown(Sniper.PressComboKey) then
    Sniper.Shrapnel()
	end
	
	if Menu.IsKeyDown(Sniper.PressComboKey) then	
...
function Sniper.OnDraw()
    ...
    local myHero = Heroes.GetLocal()
    if not myHero or NPC.GetUnitName(myHero) ~= "npc_dota_hero_sniper" then return end

    local edge_damage = 0
    local edge = NPC.GetAbility(myHero, "sniper_assassinate")
...
function Sniper.KillSteal(myHero)
    local edge = NPC.GetAbility(myHero, "sniper_assassinate")
...
function Sniper.Shrapnel()
	...
	local enemyTarget = Input.GetNearestHeroToCursor(Entity.GetTeamNum(myHero), Enum.TeamType.TEAM_ENEMY)
	local myMana = NPC.GetMana(myHero)
    local shrapnel = NPC.GetAbility(myHero, "sniper_shrapnel")

	if shrapnel and Ability.IsCastable(shrapnel, myMana) and enemyTarget ~= nil and NPC.IsPositionInRange(myHero, Entity.GetAbsOrigin(enemyTarget),1800,0) then 
	local enemyAbsOrigin = Entity.GetAbsOrigin(enemyTarget)
	Ability.CastPosition(shrapnel,enemyAbsOrigin) 
	return end	   
